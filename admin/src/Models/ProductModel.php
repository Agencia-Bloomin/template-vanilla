...
class ProductModel extends DataLayer
{
    ...
    public function lastProIdRecorded()
    {
        $lastIdFinded = 0;
        $lastProdId = (new ProductModel())->find(NULL, NULL, "id")->order("id", "desc")->limit(1)->fetch();

        if (!empty($lastProdId->id)) {
            $lastIdFinded = $lastProdId->id;
        }
        return $lastIdFinded;
    }
}
